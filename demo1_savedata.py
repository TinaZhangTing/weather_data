...
my_example_nc_file = 'ECMWF_data/_grib2netcdf-atls00-a82bacafb5c306db76464bc7e824bb75-7jGpJ5.nc'
...
df.to_csv('ECMWF_data/7jdata/time.csv')

df2=pd.DataFrame(lons,columns=['lons'])
df2.to_csv('ECMWF_data/7jdata/lons.csv')

df3= pd.DataFrame(lats,columns=['lats'])
df3.to_csv('ECMWF_data/7jdata/lats.csv')
...
x=pd.read_csv('ECMWF_data/7jdata/lons.csv')
x=x.iloc[:,1]

y=pd.read_csv('ECMWF_data/7jdata/lats.csv')
y=y.iloc[:,1]
...
#天津船航路点气象数据提取
#route=pd.read_csv('ECMWF_data/route_port_Tianjinship.csv')
#routedata=route.iloc[:,3:]
...
#    speed=fnew[lat_index,lon_index]
